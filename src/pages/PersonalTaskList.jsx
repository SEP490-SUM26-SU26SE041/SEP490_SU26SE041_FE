      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full z-50 overflow-y-auto">
        ...
        <div className="px-4 py-6 border-b border-slate-100">
          <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 p-4 border border-blue-200">
            <p className="text-xs font-bold text-blue-700 uppercase tracking-wider">T16 - Task 4</p>
            <p className="text-lg font-bold text-blue-900 mt-1">Personal Task List</p>
            <p className="text-xs text-blue-700 mt-2">Today's assignments</p>
          </div>
        </div>

        <nav className="flex-1 px-4 space-y-1 py-4">
          <button onClick={() => window.history.back()} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition">← Back</button>
          ...
          {userRole === 'Student' && (
            <button onClick={() => navigateTo('/student/morphology-entry')} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition">🌿 Morphology Entry</button>
          )}
        </nav>
